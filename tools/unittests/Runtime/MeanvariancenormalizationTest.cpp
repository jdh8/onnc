...
{
  const float   input_0[]      = {0.8439683,  0.5665144, 0.05836735, 0.02916367, 0.12964272, 0.5060197, 0.79538304,
  ...
  const float   answer_0[] = {1.3546423,  0.33053496, -1.5450814,  -1.2106764,  -0.8925952,  0.29888135, 0.38083088,
  ...
  const int32_t normalize_variance = 1;
  const int32_t across_channels    = 0;
  using dragonite::norm;

  ONNC_RUNTIME_meanvariancenormalization_float(nullptr, input_0, input_0_ndim, input_0_dims, output_0, output_0_ndim,
                                               output_0_dims, across_channels, normalize_variance);
  ASSERT_TRUE(norm(answer_0, output_0, 27) <= 1e-5 * norm(answer_0, 27));
}
