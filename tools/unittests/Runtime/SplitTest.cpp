...
SKYPAT_F(Operator_Split, test_split_equal_parts_1d) {
  ...
  const int32_t split[] = {2, 2, 2};
  const int32_t number_of_split = 3;
  ...
  ASSERT_TRUE(norm(answer_1, output_1, 2) <= 1e-5 * norm(answer_1, 2));
  ASSERT_TRUE(norm(answer_2, output_2, 2) <= 1e-5 * norm(answer_2, 2));
}
...
SKYPAT_F(Operator_Split, test_split_variable_parts_1d) {
  ...
  ONNC_RUNTIME_split_float(nullptr, input_0, input_0_ndim, input_0_dims,
                           ...
                           const_cast<int32_t *>(split), number_of_split);
  ASSERT_TRUE(norm(answer_0, output_0, 2) <= 1e-5 * norm(answer_0, 2));
  ASSERT_TRUE(norm(answer_1, output_1, 4) <= 1e-5 * norm(answer_1, 4));
}
...
SKYPAT_F(Operator_Split, test_split_equal_parts_2d) {
  ...
  ASSERT_TRUE(norm(answer_1, output_1, 6) <= 1e-5 * norm(answer_1, 6));
}
...
SKYPAT_F(Operator_Split, test_split_variable_parts_2d) {
  ...
  ASSERT_TRUE(norm(answer_1, output_1, 8) <= 1e-5 * norm(answer_1, 8));
}
