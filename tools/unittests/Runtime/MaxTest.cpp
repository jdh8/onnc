...
SKYPAT_F(Operator_Max, test_max_example) {
  ...
  ONNC_RUNTIME_max_float(nullptr, inputs, 3, input_ndims, input_shapes,
...
SKYPAT_F(Operator_Max, test_max_one_input) {
  ...
  ONNC_RUNTIME_max_float(nullptr, inputs, 1, input_ndims, input_shapes,
                         output_0, output_0_ndim, output_0_dims);
  ASSERT_TRUE(relative_error(output_0, answer_0, 3) < 1e-5f);
}
...
SKYPAT_F(Operator_Max, test_max_two_inputs) {
  ...
  ONNC_RUNTIME_max_float(nullptr, inputs, 2, input_ndims, input_shapes,
                         output_0, output_0_ndim, output_0_dims);
  ASSERT_TRUE(relative_error(output_0, answer_0, 3) < 1e-5f);
}
