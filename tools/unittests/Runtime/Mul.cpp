...
SKYPAT_F(Operator_Mul, matrix)
{
    ...
    ONNC_RUNTIME_mul_float(nullptr, a, 2, ashape, b, 2, bshape, c, 2, cshape);

    for (int i = 0; i < rows; ++i)
        for (int j = 0; j < cols; ++j)
            EXPECT_EQ(a[i] * b[j], c[i * cols + j]);

    ONNC_RUNTIME_mul_float(nullptr, a, 2, ashape, b, 1, &cols, c, 2, cshape);
